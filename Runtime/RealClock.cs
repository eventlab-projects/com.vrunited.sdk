...
public class RealClock : MonoBehaviour {
	...
	void Update () {
		...
		if(Hours)
			Hours.localRotation = Quaternion.Euler(Vector3.up * hour / 12 * 360);

		if(Minutes)
			Minutes.localRotation = Quaternion.Euler(Vector3.up * minute / 60 * 360);

		if(Seconds)
			Seconds.localRotation = Quaternion.Euler(Vector3.up * second / 60 * 360);
